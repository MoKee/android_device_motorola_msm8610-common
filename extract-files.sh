...
if [ -n "$SETUP" ]; then
    ...
    setup_vendor "$DEVICE_COMMON" "$VENDOR" "$MK_ROOT" true false
    "$MY_DIR"/setup-makefiles.sh false

    if [ -s "$MY_DIR"/../$DEVICE/proprietary-files.txt ]; then
        # Initalize the helper for device
        INITIAL_COPYRIGHT_YEAR="$DEVICE_BRINGUP_YEAR"
        setup_vendor "$DEVICE" "$VENDOR" "$MK_ROOT" false false
        "$MY_DIR"/setup-makefiles.sh false
    fi
else
    # Initialize the helper for common
    setup_vendor "$DEVICE_COMMON" "$VENDOR" "$MK_ROOT" true "$CLEANUP"
    ...
    if [ -s "$MY_DIR"/../$DEVICE/proprietary-files.txt ]; then
        ...
        INITIAL_COPYRIGHT_YEAR="$DEVICE_BRINGUP_YEAR"
        setup_vendor "$DEVICE" "$VENDOR" "$MK_ROOT" false "$CLEANUP"
